 ...
 * 设计上兼容 legacy 清单风格（features/cmds/main/preload），便于生态迁移。
 */
export const manifestSchema = z
  .object({
    /** 插件唯一 ID：小写字母数字与中划线（legacy 清单可只写 pluginName，会自动归一化） */
    name: z
      .string()
      .min(2)
      .max(64)
      .regex(/^[a-z0-9][a-z0-9-]*$/, "name 仅允许小写字母、数字与中划线"),
    /** legacy 原始字段，normalize 后仍保留 */
 ...
 * 原 input 不变，返回归一化副本。
 */
export function normalizeLegacyManifest<T>(input: T): T {
...
/** 解析并校验 plugin.json 内容（字符串或已 parse 的对象）；兼容 legacy 清单。 */
export function parseManifest(input: unknown): PluginManifest {
  const normalized =
    typeof input === "string"
      ? normalizeLegacyManifest(JSON.parse(input))
      : normalizeLegacyManifest(input);
